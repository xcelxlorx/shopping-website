...
class CartControllerTest extends IntegrationTest {
    ...
    void add_fail_test1() throws Exception {
        ...
        result.andExpect(jsonPath("$.response").isEmpty());
        result.andExpect(jsonPath("$.error.message").value("수량은 최소 1 이상 이어야 합니다."));
    ...
    void add_fail_test2() throws Exception {
        ...
        result.andExpect(jsonPath("$.success").value(false));
        result.andExpect(jsonPath("$.response").isEmpty());
        result.andExpect(jsonPath("$.error.message").value("최대 수량은 100개를 넘을 수 없습니다."));
        result.andExpect(jsonPath("$.error.status").value(400));
    }
    ...
    void add_fail_test3() throws Exception {
        ...
        List<CartRequest.SaveDTO> requestDTOs = new ArrayList<>();
        CartRequest.SaveDTO item = new CartRequest.SaveDTO();
        item.setQuantity(1); //option id 정보 누락
        ...
        result.andExpect(jsonPath("$.success").value(false));
        result.andExpect(jsonPath("$.response").isEmpty());
        result.andExpect(jsonPath("$.error.message").value("옵션 아이디는 필수 입력 값입니다."));
        result.andExpect(jsonPath("$.error.status").value(400));
    }
    ...
    void findAll_test() throws Exception {
        ...
        result.andExpect(jsonPath("$.response.products[0].carts[0].id").value(4));
    ...
    void update_fail_test1() throws Exception {
        ...
        item1.setCartId(1L);
        item1.setQuantity(-1); //10에서 -1로 수정
    ...
    void update_fail_test2() throws Exception {
        ...
        CartRequest.UpdateDTO item1 = new CartRequest.UpdateDTO();
        item1.setCartId(1L);
        item1.setQuantity(1000); //10에서 1000으로 수정
    ...
    void update_fail_test3() throws Exception {
        ...
        List<CartRequest.UpdateDTO> requestDTOs = new ArrayList<>();
        CartRequest.UpdateDTO item1 = new CartRequest.UpdateDTO();
        item1.setQuantity(1); //cart id 정보 누락
        ...
        result.andExpect(jsonPath("$.success").value(false));
        result.andExpect(jsonPath("$.response").isEmpty());
        result.andExpect(jsonPath("$.error.message").value("장바구니 아이디는 필수 입력 값입니다."));
        result.andExpect(jsonPath("$.error.status").value(400));
    }
